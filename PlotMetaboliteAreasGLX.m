function out = PlotMetaboliteAreasGLX(MPFit,conc,TITLE,outliers)
...
area.GABA = cellfun(@(x) x.GABA.area_t,MPFit);
...
[Fit.GABA,GOF.GABA] = fit(ind',area.GABA(ind)', 'a*x+b','StartPoint',[1 0]);
%[Fit.GABA_NAA,GOF.GABA_NAA] = fit((conc.GABA(ind)./conc.NAA)',1.5*(area.GABA(ind)./area.NAA(ind))','a*x+b','StartPoint',[1 0]);

out.Area = area;
out.Fit = Fit;
out.GoF = GOF;
out.mean.NAA = mean(area.NAA(ind));
out.mean.CRE = mean(area.CRE(ind));
out.CRE_NAA  = 100*out.mean.CRE/out.mean.NAA;

figure('position', [500 500 480 280]);
